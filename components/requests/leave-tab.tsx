...
import { SAMPLE_LEAVE } from "@/lib/request-data"
...
export function LeaveTab() {
  const [requests, setRequests] = useState<LeaveRequest[]>(SAMPLE_LEAVE)
  ...
  }, [requests, workerFilter, leaveTypeFilter, statusFilter])
  ...
  const hasActiveFilters = workerFilter !== "all" || leaveTypeFilter !== "all" || statusFilter !== "all"
          ...
          <Table>
            <TableHeader className="bg-slate-50 sticky top-0 z-10">
              <TableRow className="hover:bg-slate-50">
                ...
                <TableHead className="text-slate-600 text-xs font-semibold">理由</TableHead>
            ...
            <TableBody>
              {filteredRequests.map((req, index) => (
                ...
                >
                  ...
                  <TableCell className="text-sm text-slate-700">{req.startDate}</TableCell>
                  <TableCell className="text-sm text-slate-700">{req.endDate}</TableCell>
                  <TableCell className="text-sm text-slate-700 tabular-nums">{req.days}日</TableCell>
                  <TableCell className="text-sm text-slate-700 max-w-[160px]">
